...
<div class="box">
  ...
  <form action="product_process.jsp" method="post">
    ...
    <option value="travel">旅行用</option>
    <option value="boarding">登機用</option>
    <option value="family">親子友善</option>
</select>
...
<tr>
  ...
  <td>
    <div class="view-mode-<%=id%>">
      ...
      <span>
    <%
    if ("travel".equals(category)) out.print("旅行用");
    else if ("boarding".equals(category)) out.print("登機用");
    else if ("family".equals(category)) out.print("親子友善");
    else out.print(escapeHtml(category));
    %>
</span><br>
    ...
    <form action="product_process.jsp" method="post" id="form-<%=id%>" class="edit-fields" style="display:none;">
      ...
      <select name="category" required>
    <option value="travel" <%if("travel".equals(category)){%>selected<%}%>>旅行用</option>
    <option value="boarding" <%if("boarding".equals(category)){%>selected<%}%>>登機用</option>
    <option value="family" <%if("family".equals(category)){%>selected<%}%>>親子友善</option>
</select>
